<?php 
    ...
    $sql="SELECT * FROM cl_addl where status='approved' order by id desc limit $start_from,$num_per_page";
...
<body>
    ...
    <div class="view">
            <div class="well">
        <div class="table">
            <h3>Approved Products</h3><hr><br>

        <script type="text/javascript">
            function delete_id(id){
                if(confirm('sure to delete')){
                    window.location.href='approved.php?delete_id='+id;
                }
            }
            function approve_id(id){
                if(confirm('sure to approve')){
                    window.location.href='approved.php?approve_id='+id;
                }
            }
            function edit_id(id){
            ...
            if($resultCheck>0){
                while($row = mysqli_fetch_row($result)){
                    ...
                    <tr>
                        ...
                        <td style="width:60px;"><a href="javascript:approve_id(<?php echo $row[0]?>)"><button class="btn btn-primary" disabled>approved</button></a></td>
    ...
    <div class="paging">
     <?php 
                ...
                $pr_query = "SELECT * from cl_addl where status='approved'";
                ...
                if($page>1)
                {
                    echo "<a href='approved.php?page=".($page-1)."' class='btn btn-danger'>Previous</a>";
                }

                
                for($i=1;$i<=$total_page;$i++)
                {
                    echo "<a  href='approved.php?page=".$i."' class='btn btn-primary'>$i</a>";
                }

                if($i>$page)
                {
                    echo "<a href='approved.php?page=".($page+1)."' class='btn btn-danger'>Next</a>";
                }
