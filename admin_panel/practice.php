<?php 
    ...
    $sql="SELECT * FROM bidre order by id ASC limit $start_from,$num_per_page";
    $result = mysqli_query($conn, $sql);
    $resultCheck = mysqli_num_rows($result);

    if(isset($_GET['delete_id'])){
        $sql = "DELETE FROM cl_addl WHERE id =".$_GET['delete_id'];
    ...
    $sql="UPDATE cl_addl SET status='approved' WHERE id=".$_GET['approve_id']." ";
    $result = mysqli_query($conn, $sql);

    header("Location:pending.php?approved");
...
<body>
        ...
        <script type="text/javascript">
            function delete_id(id){
                if(confirm('sure to delete')){
                    window.location.href='pending.php?delete_id='+id;
                }
            }
            function approve_id(id){
                if(confirm('sure to approve')){
                    window.location.href='pending.php?approve_id='+id;
                }
            }
            function edit_id(id){
                if(confirm('sure to edit')){
                    window.location.href='cl_edit.php?edit_id='+id;
                }
            }
        </script>
        ...
        <?php 
            if($resultCheck>0){
                while($row = mysqli_fetch_row($result)){
                    ...
                    $sql = "SELECT p.*, b.bidprice bd FROM products p, bidre b WHERE b.pid='$id' AND p.id='$id' limit 1;";
                    $result = mysqli_query($conn,$sql);
                    ...
                    <tr>
                        ...
                        <form action="duration.php" method="POST">
                            <input type="" name="u_id" value="<?php echo $_SESSION['u_id'] ?>"><br><br>
                            <input type="submit" class="btn btn-success wishbtn" name="submit" value="Added in Wishlist">
                            <td style="width:60px;"><a href="javascript:approve_id(<?php echo $row[0]?>)"><button class="btn btn-primary">approve</button></a></td>
    ...
    <div class="paging">
     <?php 
                ...
                if($page>1)
                {
                    echo "<a href='pending.php?page=".($page-1)."' class='btn btn-danger'>Previous</a>";
                }

                
                for($i=1;$i<=$total_page;$i++)
                {
                    echo "<a  href='pending.php?page=".$i."' class='btn btn-primary'>$i</a>";
                }

                if($i>$page)
                {
                    echo "<a href='pending.php?page=".($page+1)."' class='btn btn-danger'>Next</a>";
                }
